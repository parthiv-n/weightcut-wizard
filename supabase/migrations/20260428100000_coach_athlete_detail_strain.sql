-- Extend coach_athlete_detail to return a 7-day strain array alongside
-- weight_7d. Same units (RPE-hours per day) and shape as the dashboard
-- sparkline, so AthleteDetail can render a larger strain card.
...
BEGIN
  ...
  SELECT json_build_object(
    ...
    'strain_7d', (
      SELECT json_agg(ROUND(COALESCE(daily.strain, 0)::numeric, 2) ORDER BY d.d)
      FROM generate_series(CURRENT_DATE - 6, CURRENT_DATE, '1 day'::interval) AS d(d)
      LEFT JOIN (
        SELECT date, SUM(COALESCE(duration_minutes, 0) * rpe / 60.0) AS strain
        FROM public.fight_camp_calendar
        WHERE user_id = p_athlete_id
          AND date >= CURRENT_DATE - 6
          AND date <= CURRENT_DATE
        GROUP BY date
      ) daily ON daily.date = d.d::date
    ),
    'today_macros', (
